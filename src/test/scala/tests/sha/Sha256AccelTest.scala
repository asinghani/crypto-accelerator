...
class Sha256AccelTest(dut: Sha256Accel) extends PeekPokeTester(dut) {
    var dataIn = Array(0l)
    ...
    for (i <- 0 until 16) {
        expect(dut.io.inputReady, 1)
        poke(dut.io.inputValid, true)
        poke(dut.io.inputData, dataIn(i))
        step(1)
        poke(dut.io.inputValid, false)
        if (Random.nextBoolean()) step(2 + Random.nextInt(8))
    }

    poke(dut.io.inputData, 0x12345678l)

    breakable {
        for (i <- 0 until 50) {
            step(1)
            if (peek(dut.io.outputValid) == 1 || peek(dut.io.inputReady) == 1) break
        }
    }

    expect(dut.io.outputValid, 1)
    for (i <- 0 until 8) {
        assert(peek(dut.io.outputData)(i) == expected(i))
    }
    ...
    for (x <- dataIn) {
        while(peek(dut.io.inputReady) == 0) step(1)
        poke(dut.io.inputValid, true)
        poke(dut.io.inputData, x)
        step(1)
        poke(dut.io.inputValid, false)
        if (Random.nextBoolean()) step(2 + Random.nextInt(8))
    }
